...
vi.mock("@/services/solutions/select-solution", () => ({
  selectUserSolutionsService: mocks.selectUserSolutionsService,
}));
...
describe("user API routes", () => {
  it("creates, updates, and deletes a user", async () => {
    ...
    expect((await json(await deleteUser(request(URLS.users, { id: UUIDS.user })))).status).toBe(
      STATUS.ok
    );
  });
  ...
  it("gets user solutions and requires the route id", async () => {
    mocks.selectUserSolutionsService.mockResolvedValueOnce(RESULTS.solutions);

    expect(
      (await json(await getUserSolutions(request(URLS.userSolutions), routeParams(UUIDS.user))))
        .body
    ).toEqual({ success: true, data: RESULTS.solutions });
    expect(
      (await json(await getUserSolutions(request(URLS.userSolutions), routeParams("")))).body
    ).toEqual({ success: false, error: "Missing required query parameter: id" });
  });
});
